use std::sync::Arc;
use egui::{
    Align, Color32, CornerRadius, CursorIcon, Layout, Margin, Rect, Sense, Stroke, Ui, containers::Frame, pos2, vec2
};
use tracing::warn;
use crate::{
    core::{
    ...
    ui::{
        ...
        icons_cache::icons, themes::colors::*,
    }
};
...
fn render_tag_button(ui: &mut Ui, state: &mut BlazeCoreState) {
    let (rect_toggle, resp_toggle) = ui.allocate_exact_size(
        vec2(45.0, 25.0),
        Sense::click()
    );
    ...
    let anim = ui.animate_bool(
        ...
        state.view_mode == ViewMode::Tags
    );

    let bg_color = COLOR_ACCENT_PURPLE;

    ui.painter()
        .rect_filled(rect_toggle, 20.0, bg_color);
