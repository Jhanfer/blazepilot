...
use egui::{pos2, vec2, Color32, Painter, Rect, Stroke, StrokeKind};
use std::sync::Arc;

pub fn render_drag_files(
    ...
    if let Some(ref target) = state.row_view.drop_target.clone() {
        if let Some(idx) = files.iter().position(|f| *f.full_path.as_ref() == **target) {
            let y =
                state.row_view.scroll_area_origin_y + idx as f32 * row_height - state.scroll_offset;
        ...
        if let Some(idx) = files
            .iter()
            .position(|f| *f.full_path.as_ref() == **target_invalid)
        {
            let y =
                state.row_view.scroll_area_origin_y + idx as f32 * row_height - state.scroll_offset;
            let target_rect = Rect::from_min_size(
