...
use egui::{
    Align, Color32, CornerRadius, Frame, Label, Layout, Margin, Rect, RichText, ScrollArea, Sense, Stroke, StrokeKind, Ui, UiBuilder, lerp, pos2, vec2
};
use tracing::{info};
use uuid::Uuid;
use crate::{
        ...
        .show(ui, |ui| {
            ...
            ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    ...
                    if let TagViewFilter::All { ref mut all_items_len } = state.tag_filter {
                    ...
                    ui.allocate_ui_with_layout(
                        ...
                        render_button(ui, "Nuevo", COLOR_BG_MAIN, COLOR_ACCENT_GLOW, Some(|| {
                            dispatcher.send(
                        ...
                        render_button(
                            ...
                            COLOR_ACCENT_GLOW, 
                            Some(|| {
                                state.tag_filter = TagViewFilter::All { all_items_len: total_items };
                            }),
                        None::<fn(&mut Ui)>
                        );
                        ...
                        ScrollArea::horizontal()
                            ...
                            .show(ui, |ui| {
                                ui.horizontal(|ui| {
                                    for (tag_name, ((tag_color, tag_id), items_len)) in tags_names.iter()
                                            .zip(tags_colors.iter()
                                                .zip(tags_ids.iter())
                                        ...
                                        let color = Color32::from_rgba_unmultiplied(
                                            vivid_color.r(), vivid_color.g(), vivid_color.b(), 60,
                                        );
                            ...
                            Frame::NONE
                                ...
                                .show(ui, |ui| {
                                    ...
                                    ui.horizontal(|ui| {
                                        ...
                                        render_button::<_, _>(ui, &tag.title, color, text_color, 
                                        None::<fn()>, 
                                            None::<fn(&mut Ui)>,
                                        );
                                        ...
                                        match ui_state.thumbnail_manager.thumb_map.try_write() {
                                            Ok(mut guard) => {
                                                tags.iter()
                                                    .flat_map(|t| &t.items)
                                                    .filter_map(|item|{
                                                        guard.get(&item.path).cloned().map(|t| (item.path.clone(), t))
                                                    })
                                                    .collect()
                                            },
                                            Err(_) => {
                                    ...
                                    ui.vertical(|ui| {
                                        ...
                                        for (i, item) in items.iter().enumerate() {
                                            ...
                                            let (rect, response) = ui.allocate_exact_size(vec2(full_width, row_height), Sense::click());

                                            let hover_t = ui.animate_bool(response.id.with("hover_rect"), response.hovered());
                                            let press_t = ui.animate_bool(response.id.with("press_rect"), response.is_pointer_button_down_on());
                                            ...
                                            let corner_radius = match (is_first, is_last) {
                                                _ if is_only => CornerRadius { nw: 0, ne: 0, sw: 20,  se: 20 },
                                                (true, _) => CornerRadius::same(0),
                                                (_, true) => CornerRadius { nw: 0,  ne: 0,  sw: 20, se: 20 },
                            ...
                            |ui| {
                                            ui.horizontal_centered(|ui| {
                                                    ...
                                                    let icon_rect = Rect::from_min_size(
                                                        pos2(
                                                            dot_rect.right() + 16.0 - (icon_size.y / 2.0),
                                                            dot_rect.center().y - (icon_size.y / 2.0),
                                                        ),
                                                        icon_size,
                                                    );

                                                    let item_vivid = ensure_min_lightness(item.color, 0.45);

                                                    let color = Color32::from_rgba_unmultiplied(
                                                        item_vivid.r(), item_vivid.g(), item_vivid.b(), 60,
                                                    );

                                                    let fixed_item_color = ensure_min_lightness(
                                                        color, 0.70
                                                    );
