...
pub fn render_grid_rubberband(
    ...
    if let (Some(start), Some(current)) = (
        ...
        let start_screen_y = content_rect.min.y + state.rubber_band.rubber_band_start_content_y
            - state.scroll_offset;
        ...
        for i in 0..points.len() {
            ...
            clipped_painter.add(egui::Shape::dashed_line(&[p1, p2], stroke, 4.0, 4.0));
        }
        ...
        for (i, _) in files.iter().enumerate() {
            ...
            let cell_x_min = state.grid_view.actual_origin.x + col as f32 * cell_size;
            let cell_y_min =
                state.grid_view.actual_origin.y + row as f32 * row_height - state.scroll_offset;
