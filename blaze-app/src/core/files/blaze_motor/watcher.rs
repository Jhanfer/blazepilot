        ...
        match event.kind {
            EventKind::Create(_) => {
                Self::handle_event_match(&event, |name| {
                    if let Err(e) = sender.send(FileLoadingMessage::FileAdded {
                        name: name.to_owned(),
                    }) {
            ...
            EventKind::Remove(_) => {
                Self::handle_event_match(&event, |name| {
                    if let Err(e) = sender.send(FileLoadingMessage::FileRemoved {
                        name: name.to_owned(),
                    }) {
                        error!("Error de enviado en watcher: {}", e);
                    }
            ...
            EventKind::Modify(_) => {
                ...
                if is_git_change {
                    ...
                    Self::handle_event_match(&event, |name| {
                        let _ = sender.send(FileLoadingMessage::FileModified {
                            name: name.to_owned(),
                        });
                    });
                }
