...
// use std::{collections::HashSet, path::{Path, PathBuf}, sync::Arc, time::{Duration, SystemTime}};
// use once_cell::sync::Lazy;
// use std::sync::Mutex;
// use tracing::{debug, warn};
// use parking_lot::Mutex as ParkingMutex;

// use crate::{app, core::bootstrap::install_manager::installation_manager::{GLOBAL_INSTALLATION_MANAGER, InstallationManager}};
...
//     fn set_installed_status(&mut self, installed: bool) {
//         self.is_installed = installed;
//         self.save().ok();
//     }

//     pub fn set_should_ask_to_install(&mut self, ask: bool) {
//         self.should_ask_to_install = ask;
//         self.save().ok();
//     }
...
//     installation_manager: Arc<ParkingMutex<InstallationManager>>,
// }
...
//         let im = GLOBAL_INSTALLATION_MANAGER.clone();

//         Self { 
//             configs: ConfigsFlags {
//                 app_ordering_mode: OrderingMode::Az,
//                 config_file_path: path,
//                 show_hidden_files: false,
//                 item_file_list_size: 10,
...
//             installation_manager: im,
//         }
//     }
    
//     fn get_im(&self)-> parking_lot::lock_api::MutexGuard<'_, parking_lot::RawMutex, InstallationManager> {
//         self.installation_manager.lock()
//     }
...
//     pub fn set_should_ask_to_install(&mut self, ask: bool) {
//         self.configs.set_should_ask_to_install(ask);
//     }

//     pub fn install(&self) {
//         self.get_im().execute_platform_tasks();
//     }

//     fn is_running_from_installation_path(&self) -> bool {
//         let im = self.get_im();
//         let current = std::env::current_exe().unwrap_or_default();
//         let system_path = PathBuf::from(im.installation_path().into_string());
//         let fallback_path = PathBuf::from(im.fallback_installation_path().into_string());

//         debug!("current: {:?}, system: {:?}, fallback: {:?}, {}", current, system_path, fallback_path, (current == system_path || current == fallback_path) && current.exists());
//         (current == system_path || current == fallback_path) && current.exists()
//     }

//     fn check_last_time_asked(tem: &mut Option<SystemTime>, interval: Duration) -> bool {
//         let should_ask = match tem {
//             None => true,
//             Some(last_instance) => {
//                 match last_instance.elapsed() {
...
//         if self.configs.should_ask_to_install {
//             if Self::check_last_time_asked(
//                 &mut self.configs.last_time_asked_installation,
//                 Duration::from_hours(24)
//             ) {
//                 self.configs.save().ok();
//                 self.configs.set_installed_status(self.is_running_from_installation_path());
//             }
//         }
